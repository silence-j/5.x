<template>
  <div class="addExtension-container">
    <el-dialog class="add-dialog" v-model="addVisible" lock-scroll="true" @close="cancelAdd">
      <span slot="title" class="dialog-header">
        <span>活动推广</span>
      </span>
      <el-row justify="center" style="margin:0 34% 20px 35%">
        <el-radio-group v-model="radio3" @change='changeTabsss'  >
					<el-radio-button label="1">社交分享</el-radio-button>
					<el-radio-button label="2">链接/二维码</el-radio-button>
			  </el-radio-group>
      </el-row>
			
      <div v-if="showtabOne">
        <div v-if='extensionShows'>
            <div class="no-datas" style="width: 100%;text-align: center;">
              <img src="../../../assets/img/monitor/noData.png" alt="">
                <p style="margin-top:32px;font-size:14px;">暂未绑定社交账号，请先绑定</p>
            </div>
          </div>
        ...
        <el-button type="primary" @click="confirmAdd" v-if="!isSending" style="width:80px;height:34px" >发送
        ...
        <el-button class="" @click="cancelAdd" style="width:80px;height:34px;border:none">取消</el-button>
        <span>推广时请选择不大于5M的图片，过大文件会导致社交发帖失败</span>
        </span>
        </div>
      </div>
      <div v-else>
        ...
        <div class="url-content">
          ...
          <div class="right">
            ...
            <div class="input-content">
              <textarea id="copyLinkUrl"  style="opacity: 0;position: absolute;top:0">{{ urlBox }}</textarea>
              <el-input v-model="urlBox" :disabled="true" class="input-xl b-title" :maxlength="160"></el-input>
              <input class="hidden-input"  :ref="'selectedInput'" type="text" v-model="urlBox">
            </div>
...
<script>
  ...
  import { getFirstImg } from '@/common/TmplContent'
  export default{
    name: 'addExtension',
    props: {
      activityOneId: String
    },
    data () {
      return {
        urlBox: '',
    ...
    computed: {
      ...
      activityAddress () {
        if (this.domainInfo.topdomain) {
          return 'http://' + this.domainInfo.topdomain + '/activity/'
        } else {
          return 'https://' + this.domainInfo.subdomain + process.env.API_HOST + '/activity/'
        }
      }
    },
    mounted () {
      this.getCompanyDomain()
      // console.log(localStorage.getItem('dialogBox'))
      // this.extensionText = 'http://www.baidu.com' + localStorage.getItem('dialogBox')
      this.getChannelList()
      let extShow = JSON.parse(localStorage.getItem('user')).channelBindingVO
      console.log(extShow.FACEBOOK === 0)
      if (extShow.FACEBOOK === 0 && extShow.LINKEDIN === 0 && extShow.PINTEREST === 0 && extShow.TWITTER === 0) {
        console.log('ss')
        this.extensionShows = true
      } else {
    ...
    watch: {
      activityOneId () {
      // console.log(this.activityOneId)
      },
      isAddExtension () {
        this.addVisible = this.isAddExtension
        this.imageUrl = getFirstImg(localStorage.getItem('imageUrl'))
        this.extensionText = this.activityAddress + localStorage.getItem('unicodeUrl')
        this.urlBox = this.activityAddress + localStorage.getItem('unicodeUrl')
        console.log(this.urlBox)
        this.imgUrl = process.env.DOMAIN + '/bshop/index/getqrcode?content=' + this.urlBox
      },
      selectAvatarList () {
        ...
        if (isComplete) {
          ...
          DevelopAPI.publishPost(params).then(res => {
            ...
            if (res.data.code === 0) {
              ...
              //  code 1 发送失败，message:twitter帖子 facebook新鲜事  推广失败。
              //  根据message 长度判断是否全部失败，不是全部失败就关闭窗口,type: "0;1;4;5;"
              let errorLen = res.data.message.split(' ').length
              let typeLen = params.type.split(';').length
              if (errorLen < typeLen) {
                this.$store.commit('DEVELOP_ADDSUCCESS', { isAddSuccess: true, sendType: params.isSchedule })
                this.cancelAdd()
              }
