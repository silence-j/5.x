    ...
    <div class="add-dialog" v-else>
      <div class="dialog-body">
        ...
        <div class="time-section">
          <span class="img-tips" v-show="!isShowTime">推广时请选择不大于5M的图片，过大文件会导致社交发帖失败</span>
      ...
      <span v-if="hiddenBtn != 'hide'" class="dialog-footer">
...
<script>
  ...
  export default{
    ...
    computed: {
      ...
      sendActivity () {
        return this.$store.state.develop.sendActivity
      }
    },
    mounted () {
      this.getChannelList()
      this.dataurl = this.activityUrl
      if (this.dataurl) {
        this.extensionText = this.dataurl
      }
      let str = this.$store.state.develop.activitySendInfo.second
      // console.log('str', getFirstImg(str))
      // console.log(str)
      this.imageUrl = getFirstImg(str)
    ...
    watch: {
      ...
      activityUrl () {
        if (!this.extensionText) {
          this.extensionText = this.activityUrl
        } else if (this.dataurl && (this.activityUrl !== this.dataurl)) {
          this.extensionText = this.extensionText.replace(this.dataurl, this.activityUrl)
        }
      },
      isAddExtension () {
