          ...
          <el-dropdown trigger="click" class="photo-dropdown" :class="isShowDropdown?'is-show':''" menu-align="start" @command="chooseMenu" v-bind:hide-on-click="false">
            ...
            <el-dropdown-menu class="photo-menu" slot="dropdown">
              <div class="menu-list" v-if="!isShowPinterest">
                <el-dropdown-item v-for="(item,index) in bindingListNew" :disabled="item.type == 5?true:false" :command="JSON.stringify(item)" :key="item">
                ...
                <i v-for="item in bindingListNew" v-if="item.type == 5" class="icon-font right-arrow" @click="choosePinterest($event)">&#xe613;</i>
              </div>
...
<!--             <el-upload
        ...
        <div class="preview-section" v-if="isPreview">
          ...
          <div>
            <div class="preview-body facebook" v-for="item in bindingListNew" v-if="(item.type == 0 || item.type == 1) && item.type == curChannel" :key="item">
              <div class="personal">
            ...
            <div class="preview-body twitter" v-for="item in bindingListNew" v-if="item.type == 4 && item.type == curChannel" :key="item">
              <div class="personal">
                <img class="avatar" :src="item.url">
              </div>
              <div class="content">
            ...
            <div class="preview-body linkedIn" v-for="item in bindingListNew" v-if="(item.type == 2 || item.type == 3) && item.type == curChannel" :key="item">
              <div class="personal">
                <img class="avatar" :src="item.url">
                <span class="name">{{item.name}}</span>
              </div>
            ...
            <div class="preview-body pinterest" v-for="item in bindingListNew" v-if="item.type == 5 && item.type == curChannel" :key="item">
              <div class="detail" v-if="imageUrl">
                <img :src="imageUrl">
              </div>
              <div class="content" v-if="extensionText">
        ...
        DevelopAPI.getChannelList().then(res => {
          if (res.data.code === 0) {
            ...
            this.channelList.forEach((value, i) => {
              if (value.bindingTag === 1) {
                ...
                if (value.channelId === 'FACEBOOK') {
                  ...
                  this.bindingListNew.push(itemObj)
                  if (value.homePageId) {
                    homeObj.type = 1
                    this.bindingList.push(homeObj)
                    this.bindingListNew.push(homeObj)
                  }
            ...
            if (obj) {
              ...
              this.bindingListNew.push(obj)
            }
            // this.bindingList.push(obj)  // 特殊处理把PINTEREST数据房列表最后位置
            this.$store.commit('DEVELOP_BINDINGLIST', { bindingList: this.bindingList })
            this.$store.commit('DEVELOP_BINDINGLIST_NEW', { bindingListNew: this.bindingListNew })
          }
          /*
           * 获取 画板列表
           * */
          this.bindingListNew.forEach((val, i) => {
            if (val.type === 5) {
